...
document.onreadystatechange = () => {
  if (document.readyState === 'interactive') {
    ...
    const partyAcronym = localStorage.getItem('clickedParty');
    const parties = JSON.parse(localStorage.getItem('parties'));
    const party = parties.find(partyList => JSON.stringify(partyList.acronym) === partyAcronym);
    localStorage.setItem('partyToEdit', JSON.stringify(party));
    ...
    const detailsHq = createNode('DIV');
